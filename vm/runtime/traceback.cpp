...
void TracebackKlass::print(HiObject* x) {
    ...
    for (int i = tbx->_stack_elements->length() - 1; i >= 0; i--) {
...
Traceback* Traceback::new_instance() {
    Handle<Traceback*> inst = new Traceback();

    auto klass = TracebackKlass::get_instance();
    inst->set_klass(klass);
    inst->_stack_elements = HiList::new_instance();
